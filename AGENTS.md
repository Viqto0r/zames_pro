...
Фреймворк — встроенный `node:test` + `node:assert`, запуск через `tsx` (без
отдельного сборщика/транспилятора). Тесты лежат в `test/*.test.ts` и проверяют
ПОВЕДЕНИЕ (контракты и результат), а не внутренние детали реализации:

- `parse-toolcall.test.ts`, `xml-toolcall.test.ts` — форматы вызова инструмента
  (чистый JSON, JSON в прозе/markdown, массив, Edit с сырым переводом строки,
  Bash с кавычками, DSML/XML, разворачивание единственного parameter args).
- `tools.test.ts` — Read/Write/Edit/Glob/Bash, sandbox (нельзя выйти выше
  рабочей директории), требование уникальности old_string в Edit, backup в undo.
- `agent-loop.test.ts` — на мок-браузере: выполнение tool-call и передача
  результата модели, respond завершает задачу, битый tool-call НЕ останавливает
  агент (просит переотправить), неизвестный инструмент, onChatReady.
- `sessions.test.ts`, `undo.test.ts`, `config.test.ts` — файловые модули и
  конфиг (HOME подменяется на временную папку; config перечитывается через
  динамический import после chdir, т.к. пути фиксируются при импорте).
- `gitTools.test.ts`, `misc.test.ts` — git-инструменты, diff, layoutInput, prompt.

ВАЖНО при написании тестов (особенности среды записи): инструмент Write может
испортить сырые `\n` и потерять `# AGENTS.md — внутреннее устройство zames
...
## Проверка изменений
 ...
 перед `{` в шаблонных строках. Поэтому в
тестах перевод строки — это `String.fromCharCode(10)` (константа `NL`), а
обращение к полю — `x['name']`, а не `x.name`. Так же собирай JSON-вызовы:
`JSON.stringify(...)` вместо ручных строк.

## Хуки (husky)

- `pre-commit`: `npm run typecheck && npm test` — коммит не пройдёт с ошибками
  типов или красными тестами.
- `pre-push`: `npm test && npm run build` — перед пушем полный прогон и сборка.
- CI: `.github/workflows/test.yml` (typecheck + test + build на push/PR).
- Публикация: `.github/workflows/publish.yml` (тег `v*` → build → npm publish).
